...
public class AlgorithmServiceImpl implements AlgorithmService {
	...
	private String adminGitEmail;

	@Value("${admin.algorithm.username}")
	private String adminGitUsername;
	...
	@Override
	public String upload(AipUser aipUser, AipAlgorithmUploadReq req, MultipartFile algorithmUploadFile) {
		...
		try {
			...
			log.error(">>>>> save algorithm error", e);
			throw e;
		}
		...
		try {
			...
			Integer pos = algorithmFileTempPath.lastIndexOf(extensionSeparator);
			...
			newAlgorithm.setOwnerId(aipUser.getUserId());
			...
			log.error(">>>>> algorithm file decompress error", e);
	...
	private void commitAndPushAlgorithm(Project project) {
		...
		try {
			...
			log.error(">>>>> git commit push error", e);
	...
	/**
	 * 알고리즘 파일을 데이터서버 특정경로에 저장한다. (algorithm.upload.path)
	 * 업로드 가능파일은 zip 확장자 파일
	 *
	 * @param algorithmCompressedFile 알고리즘 압축파일
	 * @return 압축파일 위치
	 */
	private String saveAlgorithmTempPath(MultipartFile algorithmCompressedFile) {
		...
		File uploadTempPath = Paths.get(algorithmUploadTempPath).toFile();

		if (!uploadTempPath.exists()) {
			FileUtils.mkdir(uploadTempPath);
		}

		if (uploadTempPath.listFiles().length > 0) {
			FileUtils.cleanDirectory(uploadTempPath);
		}
