...
@Slf4j
@Repository("databaseExtractorMaria")
public class DatabaseExtractorMaria extends DatabaseExtractor {
    ...
    public void setDatabase(AipDatabaseInfo databaseInfo) throws SQLException {
        ...
        try {
            ...
            ((HikariDataSource) mariaDataSource).close();
            ...
            config.setPoolName("Maria Connection Pool");

            HikariDataSource dataSource = new HikariDataSource(config);
            mariaDataSource = dataSource;

            DefaultSingletonBeanRegistry registry = (DefaultSingletonBeanRegistry) context.getBeanFactory();
            registry.destroySingleton("mariaDataSource"); //destroys the bean object
            registry.registerSingleton("mariaDataSource", dataSource); //add to singleton beans cache
    ...
    public boolean connectionTest(AipDatabaseInfo databaseInfo) {
        ...
        String databaseUrl = this.replaceDatabaseUrl(mariaUrl, databaseInfo.getDbConnIp(), databaseInfo.getDbConnPort(), databaseInfo.getDbNm());
    ...
    public List<Map<String, Object>> extractDatas(AipDatabaseInfo databaseInfo, String query, boolean isPreview) throws Exception {
        ...
        try {
            if(this.connectionTest(databaseInfo)) {
                jdbcTemplate.setDataSource(mariaDataSource);
...
//				CSVWriter writer = new CSVWriter(osw, CSVWriter.DEFAULT_SEPARATOR, CSVWriter.NO_QUOTE_CHARACTER)) {
    ...
    private boolean isEqualsDataSourceInfo(AipDatabaseInfo databaseInfo, String databaseUrl) {
        ...
        HikariDataSource dataSource = (HikariDataSource) mariaDataSource;
